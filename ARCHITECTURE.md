...
documentation, and architectural-decision mining. Not (yet): an MCP server
or a dashboard — see the root README for current scope.
...
## Structure
Modular monolith: one Cargo workspace, eight crates. Most are a layer over
...
together by `repowise-cli` (the binary). Two crates sit outside the strict
...
the raw index and commit history. No crate has been split out as a
separate service or process; there's no forcing function (scaling, team
boundary, fault isolation) that would justify it yet.
...
- Feature parity with the original repowise project's MCP server and
  dashboard — not in scope until explicitly picked up. All five of the
  original's "intelligence layers" now have CLI-facing implementations,
  though each covers only a subset of the original's scope within that
  layer (see the README for specifics per layer).
