...
| 12 | Dashboard: Architecture section restructure (Map/Explore/Coupling sub-views) | [#352](https://github.com/baileyrd/rusty_repo_wise/issues/352) | partially closed via PR #364 — Coupling sub-view: `repowise-git`'s existing `top_co_changed_pairs` (previously workspace-only) wired into a new `GET /api/coupling`, `repowise coupling`, and `get_coupling` MCP tool, plus a new dashboard section (`#/coupling`); Map (community detection over the dependency graph, laid out as a sized module map) is a genuinely new capability this port has never had and is left open, see the issue |
...
| 14 | Dashboard: Knowledge Graph view | [#354](https://github.com/baileyrd/rusty_repo_wise/issues/354) | partially closed via PR #366 — read upstream's docs directly: it's a full continuous-zoom canvas (repo→module→file→symbol) with a custom camera/culling renderer, not new backend data (file/symbol edges already exist); added the one missing layer cheaply as a `GET /api/graph-modules` (directory-level aggregation) plus a "group by module" toggle on the existing graph view, reusing its layout/rendering unchanged; the full zoomable canvas itself is left open as a much larger frontend investment, see the issue |
