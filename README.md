...
- Walk a codebase (respecting `.gitignore`), detect Rust, Python,
  TypeScript, JavaScript, Java, Kotlin, Go, and C++ files.
...
- Resolve imports and calls into a dependency graph (files and symbols as
  ...
  paths, Go's `go.mod`-anchored module paths, C++'s quote-form
  `#include "local.h"` resolved directly against the filesystem) — **not**
  full compiler-grade name resolution. Ambiguous or external references
  (npm packages/JVM classpath dependencies/Go modules outside the local
  `go.mod`/C++ angle-form `#include <system>` headers, since there's no
  `node_modules`/classpath/Go-proxy/include-path-search resolution) are
...
Only Rust, Python, TypeScript, JavaScript, Java, Kotlin, Go, and C++ are
parsed; repowise's other languages (C#, Scala, Ruby, and more)
...
- `repowise-parser` — tree-sitter-based extraction for Rust, Python,
  TypeScript, JavaScript, Java, Kotlin, Go, and C++, including
  per-function complexity/param-count/body-hash metrics.
