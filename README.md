...
- Walk a codebase (respecting `.gitignore`), detect Rust, Python,
  TypeScript, JavaScript, and Java files.
...
- Resolve imports and calls into a dependency graph (files and symbols as
  ...
  TypeScript/JavaScript's relative `./`/`../` specifiers, Java's
  Maven/Gradle `src/main/java`-anchored package paths) — **not** full
  compiler-grade name resolution. Ambiguous or external references (npm
  packages/JVM classpath dependencies, since there's no
  `node_modules`/classpath resolution) are left unresolved rather than
  guessed.
...
Only Rust, Python, TypeScript, JavaScript, and Java are parsed; repowise's
other languages (Kotlin, Go, C++, C#, Scala, Ruby, and more) aren't
...
- `repowise-parser` — tree-sitter-based extraction for Rust, Python,
  TypeScript, JavaScript, and Java, including per-function
