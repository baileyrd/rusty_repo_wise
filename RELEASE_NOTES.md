...
- Closes the last of the three parity gaps. #349 had shipped a Claude
  Code plugin; this finishes the other two hosts and fixes what #349
  got wrong.
- **Codex's hook contract is identical to Claude Code's.** Same event
  names, same `hookSpecificOutput` wrapper, same `additionalContext` /
  `permissionDecision` / `updatedInput` fields. Checked against Codex's
  own hook documentation rather than inferred from the resemblance,
  then confirmed by feeding a **Codex-shaped** payload — carrying
  `turn_id` and `tool_use_id` that Claude Code's does not — through all
  three adapters. One implementation serves both hosts unchanged; only
  the command's name was host-specific, now `agent-hook` with
  `claude-hook` kept as an alias.
- Both hosts ship as **committed project-level configs**
  (`.codex/config.toml` + `.codex/hooks.json`, `opencode.json`), so
  cloning the repo is the whole install — no plugin directory to point
  at, unlike Claude Code's.
- **#349's answer to "which hook events are cheap enough to run
  synchronously" was wrong, and measuring corrected it.** Its README
  argued `SessionStart` was cheap because `repowise status` "needs no
  git history walk". True, and never the expensive part: answering *is
  the index stale?* needs each indexed file's path, and reaching those
  meant parsing the whole index — **8.4 MB read to use 8.1 KB of it**,
  and the hook did it *twice* (once as an existence check whose result
  it discarded). Release build: **3931 ms**. `PreToolUse`'s claim did
  hold, at 6 ms.
- Fixed with `.repowise/status.json`, a ~8 KB sidecar holding exactly
  what a freshness check needs: **3931 ms -> 8 ms** (#404). Falls back
  to the full index when the sidecar is absent, older than the index,
  or an unrecognized schema version.
- `PostToolUse` (#405) needed the same trick for a different question.
  It fires after *every* matching tool call, so loading the index for a
  file's dependents would be ~2s per edit; `.repowise/dependents.json`
  (~12 KB of resolved reverse-import edges) makes it process startup
  with no measurable lookup. Matched to `Edit`/`Write` only —
  `Read`/`Grep`/`Glob` are far more frequent and a blast radius is not
  actionable then, and every match is a per-call context-token cost.
- **A vacuous test caught before it shipped.** The `PostToolUse`
  fixture first wrote `a.rs`/`b.rs` at the repo root with `mod a;`, and
  all five tests passed against an **empty** dependents map — Rust
  resolves `mod a;` to a sibling of the *crate root*, which a flat pair
  has none of. Reproduced outside the harness (`dependents.json` came
  back `{}`), then rewritten as a real crate layout that asserts the
  edge exists before testing anything depending on it.
- `repowise generate-agents-md` (#406) writes the cross-agent
  `AGENTS.md` both Codex and opencode read. `generate-claude-md
  --output AGENTS.md` half-worked already, and that was the problem:
  the block said "Generated by `repowise generate-claude-md`", pointing
  a Codex user at another agent's file. A test renders both blocks and
  diffs them with the command names normalized out, so they cannot
  drift as the block grows.
- **All of #337's federation was unreachable from inside the plugin**
  (#407). A static `.mcp.json` cannot conditionally pass `--workspace`,
  so every `repo` parameter answered *"requires a workspace"*.
  `REPOWISE_WORKSPACE` fixes it, verified by driving the MCP protocol
  before and after. Not filename discovery: the workspace TOML has no
  established name here, and inventing one would make `serve` change
  behaviour because a file appeared beside it.
- **opencode gets no hooks, deliberately.** Its plugin system is
  JavaScript/TypeScript; there is nothing a Rust CLI can install. It
  gets the MCP tools and `AGENTS.md`, and saying so beats implying
  parity it does not have.

---

## Polling auto-sync, and two bugs it exposed (closes #335) — PR #408
**2026-08-06**

- Closes #335. All five upstream auto-sync mechanisms now exist:
  post-commit hook, file watcher, GitHub webhook, GitLab webhook, and
  polling.
- **This overturns a recorded decision.** Polling had been declined for
  two reasons, and both were weaker than they read. *"`repowise watch`
  or the post-commit hook already cover it"* — those run on the machine
  where edits **happen**; polling's case is a server indexing a checkout
  something *else* updates (CI, a deploy script, another person's `git
  pull`), where the hook never fires and a self-hosted forge or a laptop
  behind NAT cannot reach a webhook URL. *"It needs persisted state this
  port lacks"* — it does not: the state is `index.indexed_commit`, and
  the identical comparison already runs on **every MCP tool call** in
  `repowise-mcp::meta::build`.
- `serve-dashboard --poll <SECONDS>` compares local HEAD against the
  indexed commit and reuses the same reindex job the dashboard button
  and both webhook receivers share. Opt-in, and **no `git fetch`** —
  fetching on a timer mutates the user's repository and needs
  credentials the server has no business holding.
- **The commit stamp was being erased** (also #345).
  `repowise_parser::build_index` leaves `indexed_commit` as `None`;
  only `repowise init`/`update` stamped it. So the dashboard button and
  *both webhook receivers* had been wiping it since #345, degrading the
  MCP staleness warning from a commit comparison to an age guess. For
  polling it was fatal rather than degrading — with nothing to compare,
  it fired exactly once and went silent forever. The unit test passed
  anyway; running it against a real repo across two commits is what
  caught it.
- **The dependents sidecar was left stale.** #405 routed the CLI's four
  index-write sites through one helper *precisely* so none could forget
  it — and missed the server's reindex job, a fifth site. A dashboard
  reindex left `dependents.json` older than the index, which
  `load_dependents` correctly refuses, silently disabling the
  `PostToolUse` hook until someone ran the CLI.
- Both regressions are covered by tests confirmed to **fail when the fix
  is removed**, rather than assumed to cover it.

---

## Dashboard performance: 19s to milliseconds (closes #398) — PRs #400, #401, #402, #403
**2026-08-06**

- Closes #398. `/api/files` **16.0s -> 0.004s**, `/api/health` **19.0s
  -> 0.015s**, `/api/overview` **3.1s -> 0.005s**, `/api/hotspots`
  **3.0s -> 0.038s** — all first-request numbers against a freshly
  started server.
- **Profiling reversed the issue's own fix order, twice.** #398 ranked
  index caching first, assuming a ~3s load+build floor. Measured,
  `RepoGraph::build` is **0.11s** — no floor at all — and one health
  check was the whole story: `find_near_duplicates` at **12.2s of a
  12.4s** analysis, against 0.00s for god-classes and LCOM4.
- `near_duplicate.rs` claimed its Rabin-Karp bucketing meant "pairs with
  nothing in common never get compared at all". True, and useless: at a
  3-token window almost every pair of functions shares *something*. The
  largest window bucket held **1308 of 2089** eligible symbols, and the
  scan cost **16.9M pair-visits** — it had degenerated into repeated
  all-pairs work (#400).
- **The cap does all the work, not the restructuring.** With it
  disabled, computing exact intersections over all 16.9M pairs takes
  **18.6s — slower than the 12.4s it replaced**. There is no purely
  mechanical version of this fix.
- **It changes what the marker reports**, and that is recorded rather
  than buried: **28% fewer pairs** (7,246 of 25,510), verified by
  diffing both implementations' complete output. Nothing is added — a
  strict subset — and every dropped pair's entire shared content is
  repo-wide boilerplate. The highest-ratio casualty is 0.88 between two
  unrelated four-line tests sharing `tempfile::tempdir().unwrap()`.
- **Second reversal:** after that fix, near-duplicate detection was
  *still* 98% of health analysis, and the remainder was not disk I/O
  (0.63s of 3.46s) — which is what made caching, not further
  micro-optimising, the right answer (#402).
- Index and health caches are keyed per repo root, because #337 made
  every endpoint answer from an arbitrary workspace repo. The two health
  shapes cannot share an invalidation key: the plain report is a pure
  function of the index, but the org-signals report is built from `git
  blame` output that changes on a new commit *without* touching
  `.repowise/index.json`, so it carries the head SHA too.
- Startup warming (#403) closed the remaining gap — the caches were
  fastest precisely when nobody was waiting, and slowest when someone
  was. Only the server's own root; workspace members stay lazy, since
  eagerly indexing every configured repo would make startup a long CPU
  burn for repos that may never be opened.
- Cache-invalidation tests were confirmed to fail when the mtime check
  or key is broken, and the mtime one sets the new timestamp
  **explicitly** — a filesystem with one-second granularity would
  otherwise let a fast test rewrite the index, get an identical
  timestamp, and pass for entirely the wrong reason.

---

## The dashboard stylesheet — PR #397
**2026-08-05**

- The dashboard looked unstyled because **it had no stylesheet at all**:
  no `.css` file anywhere in the repo, no `<link>` in `index.html`, no
  inline `<style>`. Every view rendered on browser defaults.
- Meanwhile the frontend had carried style hooks since it was written —
  **138 `class=` sites across 18 names** (`empty` on 73, `error` on 41,
  plus `view-nav`, `badge`, `repo-card`, `mono`, ...), plus a
  `class:active` on the nav links. The hooks were written; the CSS never
  was.
- Two markup changes only, both wrappers around existing elements: a
  `header` so the title/selector/reindex/present/search stop stacking,
  and a `main` so content takes a readable max-width. Everything else is
  CSS against hooks already present.
- Numeric table columns are deliberately left un-right-aligned: the
  markup has no per-column hook, and targeting by child index would
  break the moment a column moves.
- Both colour schemes, verified by asserting computed colours rather
  than eyeballing.

---

## Federated workspace queries (closes #337) — PRs #391, #393, #394, #395, #396, #399
**2026-08-05**

- Closes #337. `root` no longer stays fixed for the life of the
  process: **19 MCP tools**, none on a fixed root (17 take `repo`, 2 are
  workspace-level by construction), and **36 dashboard endpoints** — 6
  federate, 22 scope, 8 legitimately take none.
- **`repo` means two things**, depending on whether the answer can carry
  a label. Where results are a list whose rows can each name their repo,
  `"all"` federates. Where the answer is a single subject (one file, one
  symbol, one LLM reply) or a list of un-labeled rows, `repo` selects
  *which* repo and `"all"` is refused with a reason rather than quietly
  answering from one of them.
- Two ranking decisions follow the precedent `/api/overview` set by
  declining to merge `most_depended_on`: federated search applies its
  limit per repo, and `get_risk` applies `top_n` per repo — a hotspot
  score is churn × complexity, and churn tracks a repo's own age and
  commit rate, so one merged ranking would let the oldest, busiest repo
  fill the list.
- **The repo selector shipped a bug that mixed two repos in one view.**
  The frontend injects `?repo=` into every GET from one choke point, but
  only six endpoints declared the parameter — the rest let serde discard
  it and answered from the server's own root. Against a two-repo
  workspace, `?repo=billing` (2 files) returned billing's Overview
  beside the *other* repo's Files (123), Hotspots (15), Graph (123
  nodes) and Symbols (2960), with nothing indicating the mismatch.
- **Three frontend render bugs found only by driving a browser** (#396),
  after #395's body had correctly said the selector was compile- and
  logic-verified only. The `<select>` never showed its value (options
  render inside a `Suspense`, appended after the node, so both
  `selected=` and `prop:value` had nothing to match); hash-only
  navigation dropped `?repo=` entirely; and the dropdown went stale
  because the component is created once. All three shared a shape: the
  *data* was right and its *label* was wrong.
- `/api/search` was resolved additively rather than by changing a type:
  `files` stays a bare string list, and a new `matches` array carries
  the same paths *with* their repo. Its test searches a fixture with
  `src/f0.rs` in **both** repos and asserts `files` is the ambiguous
  `["src/f0.rs", "src/f0.rs"]` while `matches` distinguishes them — so
  the thing `matches` exists to solve is what the test pins.
- `get_risk` collected git analytics from the MCP server's own root
  rather than the repo under assessment. Unreachable before `repo`
  existed, since those were always the same path — adding the parameter
  is what would have made it reachable.
- A CHANGELOG line claiming four endpoints meant "every federatable
  endpoint now honours it" was corrected: it was 6 of 30.
...
- Removes `repowise dashboard`/`repowise-dashboard` (the crate directory,
  workspace member, and CLI command) entirely. The static dashboard wrote
  one self-contained `.repowise/dashboard/index.html` — regenerated by
  re-running the command, no auto-refresh, no live search, drill-down
  links only if `repowise docs` had already run. The live dashboard
  (`repowise serve-dashboard`, `repowise-server` + `repowise-web`) has
  covered every one of those sections since closing out issues #59/#65,
  and now exceeds it on every axis: live search, per-file drill-down
  panels, a dependency graph, chat, Present Mode, a live reindex job
  banner, Settings, Usage, and multi-repo workspace views once
  `--workspace` is configured. Keeping both meant two dashboards to keep
  in sync on every future feature; this keeps one.
- `repowise status`'s "dashboard: generated/not generated" line is
  removed too — it checked for the now-gone static file specifically,
  and has nothing left to report.
- Doc-comment analogies elsewhere in the codebase that referenced
  `repowise-dashboard` (in `repowise-mcp`'s `get_risk`, `repowise-llm`'s
  wiki-summary generation) now point at the live dashboard's equivalent
  behavior (`/api/hotspots`'s same git-degrade tradeoff,
  `repowise-web`'s drill-down panels) instead.
- README.md's "Dashboard" section is now the one dashboard section (was
  two: "Dashboard" for the static page, "Live dashboard server" for the
  live one), with a migration note explaining what was removed and why.
  ARCHITECTURE.md's now-dead description of the static page's internal
  data flow is removed; its crate-dependency table drops the
  `repowise-dashboard` mention.
- `cargo test --workspace`/`fmt`/`clippy` all clean; manually confirmed
  `repowise --help` no longer lists `dashboard`/mentions a static
  dashboard anywhere in its own about text.
