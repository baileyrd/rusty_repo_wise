use crate::metrics;
use crate::util::text;
use repowise_core::{CallRef, FileRecord, ImportRef, Language, Symbol, SymbolKind};
use std::path::Path;
use tree_sitter::{Node, Parser};

pub fn extract(path: &Path, source: &str) -> anyhow::Result<FileRecord> {
    let mut parser = Parser::new();
    parser.set_language(&tree_sitter_kotlin_ng::LANGUAGE.into())?;
    let tree = parser
        .parse(source, None)
        .ok_or_else(|| anyhow::anyhow!("tree-sitter failed to parse {}", path.display()))?;

    let mut symbols = Vec::new();
    let mut imports = Vec::new();
    let mut calls = Vec::new();

    let mut walker = Walker {
        path,
        source,
        symbols: &mut symbols,
        imports: &mut imports,
        calls: &mut calls,
        scope_stack: Vec::new(),
        class_stack: Vec::new(),
    };
    walker.visit(tree.root_node());

    Ok(FileRecord {
        path: path.to_path_buf(),
        language: Language::Kotlin,
        lines: source.lines().count(),
        symbols,
        imports,
        calls,
        field_accesses: Vec::new(),
    })
}

struct Walker<'a> {
    path: &'a Path,
    source: &'a str,
    symbols: &'a mut Vec<Symbol>,
    imports: &'a mut Vec<ImportRef>,
    calls: &'a mut Vec<CallRef>,
    scope_stack: Vec<String>,
    /// Stack of enclosing class/interface/object names, innermost last.
    class_stack: Vec<String>,
}

impl<'a> Walker<'a> {
    fn current_scope(&self) -> Option<String> {
        self.scope_stack.last().cloned()
    }

    fn line_of(&self, node: Node) -> usize {
        node.start_position().row + 1
    }

    fn visit(&mut self, node: Node) {
        match node.kind() {
            // The grammar uses `class_declaration` for both `class` and
            // `interface` — the distinguishing keyword is a direct child,
            // not a separate node kind or field.
            "class_declaration" | "object_declaration" => {
                if let Some(name_node) = node.child_by_field_name("name") {
                    let name = text(name_node, self.source).to_string();
                    let start_line = self.line_of(node);
                    let end_line = node.end_position().row + 1;
                    let kind = if has_child_kind(node, "interface") {
                        SymbolKind::Trait
                    } else {
                        SymbolKind::Class
                    };
                    self.symbols.push(Symbol {
                        id: Symbol::make_id(self.path, &name, start_line),
                        name: name.clone(),
                        kind,
                        file: self.path.to_path_buf(),
                        start_line,
                        end_line,
                        parent: None,
                        complexity: 0,
                        max_nesting_depth: 0,
                        param_count: 0,
                        body_hash: None,
                    });
                    self.class_stack.push(name);
                    self.visit_children(node);
                    self.class_stack.pop();
                    return;
                }
            }
            "function_declaration" => {
                if let Some(name_node) = node.child_by_field_name("name") {
                    let name = text(name_node, self.source).to_string();
                    let start_line = self.line_of(node);
                    let end_line = node.end_position().row + 1;
                    let id = Symbol::make_id(self.path, &name, start_line);
                    let parent = self.class_stack.last().cloned();
                    // Interface method signatures (no body) are still
                    // recorded, just with 0 complexity — same treatment
                    // as Rust/Java's bodiless method signatures.
                    let body = find_child(node, "function_body");
                    let complexity = body
                        .map(|b| {
                            metrics::cyclomatic_complexity(
                                b,
                                |n| is_decision(n, self.source),
                                is_nested_function,
                            )
                        })
                        .unwrap_or(0);
                    let max_nesting_depth = body
                        .map(|b| {
                            metrics::max_nesting_depth(
                                b,
                                |n| is_decision(n, self.source),
                                is_nested_function,
                            )
                        })
                        .unwrap_or(0);
                    let params = find_child(node, "function_value_parameters");
                    let param_count = metrics::count_params(params);
                    let body_hash = body.and_then(|b| metrics::body_hash(b, self.source));
                    self.symbols.push(Symbol {
                        id: id.clone(),
                        name,
                        kind: SymbolKind::Method,
                        file: self.path.to_path_buf(),
                        start_line,
                        end_line,
                        parent,
                        complexity,
                        max_nesting_depth,
                        param_count,
                        body_hash,
                    });
                    self.scope_stack.push(id);
                    self.visit_children(node);
                    self.scope_stack.pop();
                    return;
                }
            }
            "import" => {
                let line = self.line_of(node);
                let path_text = import_path_text(node, self.source);
                if !path_text.is_empty() {
                    self.imports.push(ImportRef {
                        path: path_text,
                        line,
                        resolved_file: None,
                    });
                }
            }
            // Kotlin has no `new` keyword — `Widget()` parses as an
            // ordinary `call_expression`, so class instantiation is
            // already covered here without a separate node kind.
            "call_expression" => {
                if let Some(target) = node.child(0) {
                    let callee_name = call_target_name(target, self.source);
                    if !callee_name.is_empty() {
                        self.calls.push(CallRef {
                            caller: self.current_scope(),
                            callee_name,
                            line: self.line_of(node),
                        });
                    }
                }
            }
            _ => {}
        }
        self.visit_children(node);
    }

    fn visit_children(&mut self, node: Node) {
        let mut cursor = node.walk();
        for child in node.children(&mut cursor) {
            self.visit(child);
        }
    }
}

/// Does `node` have a direct child of kind `kind`? Used to tell `class`
/// from `interface` declarations, which the grammar folds into the same
/// `class_declaration` node.
fn has_child_kind(node: Node, kind: &str) -> bool {
    let mut cursor = node.walk();
    let found = node.children(&mut cursor).any(|c| c.kind() == kind);
    found
}

/// This grammar doesn't attach field names to a function's parameter
/// list or body, unlike Rust/Python/Java — find them by node kind instead.
fn find_child<'t>(node: Node<'t>, kind: &str) -> Option<Node<'t>> {
    let mut cursor = node.walk();
    let found = node.children(&mut cursor).find(|c| c.kind() == kind);
    found
}

/// For `obj.method()` (a `navigation_expression`) return `method`; for a
/// bare `func()`/`Widget()` return the identifier itself.
fn call_target_name(node: Node, source: &str) -> String {
    match node.kind() {
        "navigation_expression" => {
            let mut cursor = node.walk();
            let identifiers: Vec<Node> = node
                .children(&mut cursor)
                .filter(|c| c.kind() == "identifier")
                .collect();
            identifiers
                .last()
                .map(|n| text(*n, source).to_string())
                .unwrap_or_default()
        }
        "identifier" => text(node, source).to_string(),
        _ => String::new(),
    }
}

/// The dotted import path, e.g. `com.example.util.Helper`, or
/// `com.example.other.*` for a wildcard import (the `.*` suffix parses as
/// a trailing sibling token, not part of the preceding identifier, same
/// shape as Java's grammar). The `as` alias, if any, doesn't affect the
/// resolved path.
fn import_path_text(node: Node, source: &str) -> String {
    let mut cursor = node.walk();
    let mut path = String::new();
    let mut is_wildcard = false;
    for child in node.children(&mut cursor) {
        match child.kind() {
            "qualified_identifier" | "identifier" if path.is_empty() => {
                path = text(child, source).to_string();
            }
            "*" => is_wildcard = true,
            _ => {}
        }
    }
    if is_wildcard {
        path.push_str(".*");
    }
    path
}

/// Cyclomatic-complexity decision points for Kotlin: `if`-expressions
/// (also Kotlin's ternary-equivalent), loops, exception handlers, `when`
/// branches (not `else`), and short-circuiting boolean operators.
fn is_decision(n: Node, source: &str) -> bool {
    match n.kind() {
        "if_expression" | "for_statement" | "while_statement" | "catch_block" => true,
        "when_entry" => !text(n, source).trim_start().starts_with("else"),
        "binary_expression" => n
            .child_by_field_name("operator")
            .map(|op| matches!(text(op, source), "&&" | "||"))
            .unwrap_or(false),
        _ => false,
    }
}

/// Only nested named function declarations get their own symbol; a
/// lambda/anonymous function passed as an argument doesn't, so its
/// branches fold into the enclosing scope's count, same tradeoff as
/// Rust's untracked closures.
fn is_nested_function(n: Node) -> bool {
    n.kind() == "function_declaration"
}

#[cfg(test)]
mod tests {
    use super::*;

    fn extract_str(source: &str) -> FileRecord {
        extract(Path::new("Test.kt"), source).unwrap()
    }

    #[test]
    fn extracts_class_interface_object_and_method() {
        let rec = extract_str(
            "class Widget {\n  fun area(): Int {\n    return helper()\n  }\n}\n\ninterface Shape {\n  fun area(): Int\n}\n\nobject Registry {\n  fun get() {}\n}\n",
        );
        let widget = rec.symbols.iter().find(|s| s.name == "Widget").unwrap();
        assert_eq!(widget.kind, SymbolKind::Class);

        let shape = rec.symbols.iter().find(|s| s.name == "Shape").unwrap();
        assert_eq!(shape.kind, SymbolKind::Trait);

        let registry = rec.symbols.iter().find(|s| s.name == "Registry").unwrap();
        assert_eq!(registry.kind, SymbolKind::Class);

        let area = rec
            .symbols
            .iter()
            .find(|s| s.name == "area" && s.parent.as_deref() == Some("Widget"))
            .unwrap();
        assert_eq!(area.kind, SymbolKind::Method);

        assert_eq!(rec.calls.len(), 1);
        assert_eq!(rec.calls[0].callee_name, "helper");
        assert_eq!(rec.calls[0].caller, Some(area.id.clone()));
    }

    #[test]
    fn extracts_plain_aliased_and_wildcard_imports() {
        let rec = extract_str(
            "import com.example.util.Helper\nimport com.example.util.Widget as W\nimport com.example.other.*\n\nclass C\n",
        );
        let paths: Vec<_> = rec.imports.iter().map(|i| i.path.as_str()).collect();
        assert!(paths.contains(&"com.example.util.Helper"));
        assert!(paths.contains(&"com.example.util.Widget"));
        assert!(paths.contains(&"com.example.other.*"));
    }

    #[test]
    fn records_bare_invocation_as_a_call_to_the_class() {
        let rec = extract_str(
            "class Widget\n\nclass Factory {\n  fun make(): Widget {\n    return Widget()\n  }\n}\n",
        );
        let make = rec.symbols.iter().find(|s| s.name == "make").unwrap();
        let call = rec
            .calls
            .iter()
            .find(|c| c.callee_name == "Widget")
            .unwrap();
        assert_eq!(call.caller, Some(make.id.clone()));
    }

    #[test]
    fn computes_cyclomatic_complexity_and_param_count() {
        let rec = extract_str(
            "fun straightLine(a: Int, b: Int): Int {\n  return a + b\n}\n\nfun branchy(x: Int, y: Int, z: Int): Int {\n  if (x > 0 && y > 0) {\n    return 1\n  } else if (z > 0) {\n    return 2\n  }\n  for (i in 0..x) {\n    if (i == y) {\n      return i\n    }\n  }\n  return 0\n}\n",
        );
        let straight = rec
            .symbols
            .iter()
            .find(|s| s.name == "straightLine")
            .unwrap();
        assert_eq!(straight.complexity, 1);
        assert_eq!(straight.param_count, 2);

        let branchy = rec.symbols.iter().find(|s| s.name == "branchy").unwrap();
        // base(1) + if(1) + &&(1) + else-if(1) + for(1) + if(1) = 6
        assert_eq!(branchy.complexity, 6);
        assert_eq!(branchy.param_count, 3);
    }

    #[test]
    fn hashes_duplicate_function_bodies_identically() {
        let rec = extract_str(
            "fun one(n: Int): Int {\n  var total = 0\n  for (i in 0..n) {\n    total += i\n  }\n  return total\n}\n\nfun two(n: Int): Int {\n  var total = 0\n  for (i in 0..n) {\n    total += i\n  }\n  return total\n}\n\nfun shortFn(): Int {\n  return 1\n}\n",
        );
        let one = rec.symbols.iter().find(|s| s.name == "one").unwrap();
        let two = rec.symbols.iter().find(|s| s.name == "two").unwrap();
        let short = rec.symbols.iter().find(|s| s.name == "shortFn").unwrap();

        assert!(one.body_hash.is_some());
        assert_eq!(one.body_hash, two.body_hash);
        assert!(short.body_hash.is_none());
    }

    #[test]
    fn interface_method_signature_has_no_body_but_is_still_a_symbol() {
        let rec = extract_str("interface Shape {\n  fun area(): Int\n}\n");
        let area = rec.symbols.iter().find(|s| s.name == "area").unwrap();
        assert_eq!(area.complexity, 0);
        assert_eq!(area.parent.as_deref(), Some("Shape"));
    }
}
